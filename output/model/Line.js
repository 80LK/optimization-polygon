...
class Line {
    ...
    _distToPoint(point) {
        ...
        if (c1 <= 0)
            return point.distToPoint(this.start);
        const c2 = thisV.dot(thisV);
        if (c2 <= c1)
            return point.distToPoint(this.end);
        const b = c1 / c2;
    ...
    distToPoint(point) {
        ...
        if (r1 >= z.distToPoint(new Point(r2, r12))) {
            ...
            let a = this.end.y - this.start.y, b = this.start.x - this.end.x, c = -this.start.x * (this.end.y - this.start.y) + this.start.y * (this.end.x - this.start.x);
