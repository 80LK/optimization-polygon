...
class Polygon {
    ...
    _havePoint(point) {
        ...
        let j = l - 1;
        for (let i = 0; i < l; i++) {
            if ((this._points[i].y < point.y && this._points[j].y >= point.y || this._points[j].y < point.y && this._points[i].y >= point.y) ||
    ...
    static isect_line(p1, p2, pos, winding) {
        let x1 = p1.x;
        let y1 = p1.y;
        let x2 = p2.x;
        let y2 = p2.y;
    ...
    graham() {
        ...
        points.shift();
        points.sort((a, b) => {
            const aA = p0.polarAngle(a), aB = p0.polarAngle(b), less = -1, large = 1;
            if (aA < aB)
                return less;
            if (aA > aB)
                return large;
            const dA = p0.distToPoint(a), dB = p0.distToPoint(a);
            if (dA < dB)
                return less;
            if (dA > dB)
                return large;
